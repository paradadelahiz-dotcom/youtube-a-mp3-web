...
def yt_info(url):
    ...
    return {'id':data.get('id'),'title':data.get('title') or 'Vídeo de YouTube','thumbnail':data.get('thumbnail'),'duration':data.get('duration'),'channel':data.get('uploader') or data.get('channel') or 'YouTube','success':True}
...
def exe_url():
    if not EXE_DOWNLOAD_URL:
        return jsonify(error='Falta configurar EXE_DOWNLOAD_URL en las variables de Railway.',success=False),503
    return jsonify(url=EXE_DOWNLOAD_URL,success=True)
